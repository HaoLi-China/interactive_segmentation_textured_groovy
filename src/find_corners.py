...
concave_corner_directions.append(vec1)
	...
	#cv.ShowImage("corner_finder", col3)
       ...
       def findCornersService(self,req):
        ...
        #iplImage = cv.LoadImage('/home/hao/test.png')
        #test_img = cv.CreateMat(iplImage.height, iplImage.width, cv.CV_8U)
        #cv.CvtColor(iplImage, test_img, cv.CV_BGR2GRAY)
        
        cv.ShowImage("iplImage", iplImage)
        cv.WaitKey()
        ...
        print "test3"
        res = interactive_segmentation_textured.srv.cornerFindResponse()
        ...
        for i in range(len (convex_corners)):
            ...
            for i in range(len (concave_corners)):
                ...
                if __name__ == '__main__':
                    ...
                    rospy.sleep(1.0)
                    cf = CornerFinder()
                    s = rospy.Service('find_corners',  interactive_segmentation_textured.srv.cornerFind, lambda a: cf.findCornersService(a) )
                    rospy.spin()

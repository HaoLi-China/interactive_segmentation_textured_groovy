...
###############
# This code was partly taken from ttps://code.ros.org/trac/opencv/browser/trunk/opencv/samples/python/lkdemo.py?rev=2744  For license information please see the package LICENSE file
...
class FeatureTracker:
    ...
    def __init__(self):
        ...
        self.estimateRigidServer = rospy.ServiceProxy("estimateRigid", interactive_segmentation_textured.srv.estimateRigid())
        self.getDepthServer = rospy.ServiceProxy("getDepthImage", interactive_segmentation_textured.srv.depthImage())
        self.ROI = [0,412,2190,1424] #roi for prosilica x,y,width,height
        self.ROI = [0,0,640,480] #roi for prosilica x,y,width,height
...
#        self.features = features
        ...
        for (f_old, f_new) in zip(features_old, features):
            cv.Line(image, (int(f_old[0]), int(f_old[1])), (int(f_new[0]),int(f_new[1])), (0,0,255,0),1)
...
#def display(feature_tracker):
    ...
    def estimateRigidTransformationRANSAC(self, all_features_old, all_features  ):
        ...
        req = interactive_segmentation_textured.srv.estimateRigidRequest()
